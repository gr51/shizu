...
export const TRIGGER_EVENTS = new Set(['onFirstBlood', 'onEliteKill', 'onBossKill', 'onChestOpen', 'onStageClear', 'onLowHp', 'onTimeTick', 'onSurgeSpawn', 'onShopOpen', 'onChoiceOpen', 'onPlaneEnter', 'onDevour', 'onAmbushSpawn', 'onBossHalfHp', 'onMiniRushClear', 'onGearDrop', 'onCrisis', 'onEndlessLayer', 'onBossSpawn']);
...
export class Run {
  ...
  constructor(save, dungeon, seed) {
    ...
    this.runPlaneTriggers('onPlaneEnter');
  }
  ...
  runPlaneTriggers(eventName, only = null) {
    ...
    for (const t of triggers) {
      ...
      for (const act of Array.isArray(t.actions) ? t.actions : []) {
        ...
        } else if (act.type === 'permHp' && Number(act.pct) > 0) {
          this.save.player.permHpPct = (this.save.player.permHpPct ?? 0) + Math.min(10, Number(act.pct));
          this.emit(`⚡ 触发器：永久生命 +${Math.min(10, act.pct)}%（跨局）`, 'win');
        } else if (act.type === 'permAtk' && Number(act.pct) > 0) {
          this.save.player.permAtkPct = (this.save.player.permAtkPct ?? 0) + Math.min(10, Number(act.pct));
          this.emit(`⚡ 触发器：永久攻击 +${Math.min(10, act.pct)}%（跨局）`, 'win');
        } else if (act.type === 'essence' && Number(act.amount) > 0) {
          this.save.player.gearEssence = (this.save.player.gearEssence ?? 0) + Math.min(300, Math.round(Number(act.amount)));
          this.emit(`⚡ 触发器：装备精华 +${Math.min(300, Math.round(act.amount))}`, 'gene');
        } else if (act.type === 'freeBanish' && Number(act.count) > 0) {
          this.banishUsed = Math.max(0, (this.banishUsed ?? 0) - Math.min(2, Math.round(Number(act.count))));
          this.emit(`⚡ 触发器：放逐次数 +${Math.min(2, Math.round(act.count))}`, 'info');
        ...
        } else if (act.type === 'buffGeneBonus' && Number(act.pct) > 0) {
          this.stats.geneBonus = (this.stats.geneBonus ?? 0) + Math.min(2, Number(act.pct));
          this.emit(`⚡ 触发器：基因产出永久 +${Math.round(Math.min(2, act.pct) * 100)}%`, 'gene');
        } else if (act.type === 'buffChain' && Number(act.count) > 0) {
          this.stats.chainJumps = (this.stats.chainJumps ?? 0) + Math.min(5, Math.round(Number(act.count)));
          this.emit(`⚡ 触发器：雷链弹射永久 +${Math.min(5, Math.round(act.count))} 跳`, 'win');
        } else if (act.type === 'buffDot' && Number(act.pct) > 0) {
          this.stats.dotMul = (this.stats.dotMul ?? 0) + Number(act.pct);
          this.emit(`⚡ 触发器：持续伤害永久 +${Math.round(Number(act.pct) * 100)}%`, 'win');
        } else if (act.type === 'buffCooldown' && Number(act.pct) > 0) {
          this.stats.cooldown = Math.max(0.4, (this.stats.cooldown ?? 1) * (1 - Math.min(0.6, Number(act.pct))));
          this.emit(`⚡ 触发器：技能冷却永久 -${Math.round(Math.min(0.6, act.pct) * 100)}%`, 'win');
        } else if (act.type === 'buffDmg' && Number(act.pct) > 0) {
          this.stats.atk *= 1 + Math.min(2, Number(act.pct));
          this.emit(`⚡ 触发器：总伤害永久 +${Math.round(Math.min(2, act.pct) * 100)}%`, 'win');
        ...
        } else if (act.type === 'buffCritDmg' && Number(act.pct) > 0) {
          this.stats.critDmg = (this.stats.critDmg ?? 0) + Number(act.pct);
          this.emit(`⚡ 触发器：暴击伤害永久 +${Math.round(Number(act.pct) * 100)}%`, 'win');
        } else if (act.type === 'buffSuck' && Number(act.pct) > 0) {
          this.stats.suckRadius = (this.stats.suckRadius ?? 1) * (1 + Math.min(2, Number(act.pct)));
          this.emit(`⚡ 触发器：吸取半径永久 +${Math.round(Math.min(2, act.pct) * 100)}%`, 'win');
        } else if (act.type === 'buffAspd' && Number(act.pct) > 0) {
          this.stats.aspd *= 1 + Math.min(1, Number(act.pct));
          this.emit(`⚡ 触发器：攻速永久 +${Math.round(Math.min(1, act.pct) * 100)}%`, 'win');
        } else if (act.type === 'buffRange' && Number(act.pct) > 0) {
          this.stats.range *= 1 + Math.min(2, Number(act.pct));
          this.emit(`⚡ 触发器：攻击范围永久 +${Math.round(Math.min(2, act.pct) * 100)}%`, 'win');
        } else if (act.type === 'buffAoe' && Number(act.pct) > 0) {
          this.stats.aoe = (this.stats.aoe ?? 0) + Number(act.pct);
          this.emit(`⚡ 触发器：清场范围永久 +${Math.round(Number(act.pct) * 100)}%`, 'win');
        } else if (act.type === 'buffDmgReduct' && Number(act.pct) > 0) {
          this.stats.dmgReduct = Math.min(0.9, (this.stats.dmgReduct ?? 0) + Number(act.pct));
          this.emit(`⚡ 触发器：永久减伤 +${Math.round(Number(act.pct) * 100)}%`, 'win');
        } else if (act.type === 'buffLifesteal' && Number(act.pct) > 0) {
          this.stats.lifesteal = Math.min(0.3, (this.stats.lifesteal ?? 0) + Number(act.pct));
          this.emit(`⚡ 触发器：吸血 +${Math.round(Number(act.pct) * 100)}%`, 'win');
        } else if (act.type === 'buffCrit' && Number(act.pct) > 0) {
          this.stats.crit = Math.min(1, (this.stats.crit ?? 0.05) + Number(act.pct));
          this.emit(`⚡ 触发器：暴击率 +${Math.round(Number(act.pct) * 100)}%`, 'win');
        } else if (act.type === 'buffRegen' && Number(act.pct) > 0) {
          this.stats.regen = (this.stats.regen ?? 0) + Math.min(0.05, Number(act.pct) / 100);
          this.emit(`⚡ 触发器：每秒回血 +${Number(act.pct)}% 上限`, 'win');
  ...
  openShop() {
    ...
    this.runPlaneTriggers('onShopOpen');
    return true;
  }
