// ===== planes.js · 12 位面副本模板（关卡策划 + 数值平衡表 五章）=====
// 数值为战力 1 基准；实际 = 基准 × D × 阶段系数 × 动态系数
// route 为 null 表示无专属路线（奇巧迷宫/诸天之心）
...
export function findPlane(name) {
  return planes.find((p) => p.name === name);
}

export function findPlaneByRoute(routeId) {
  return planes.find((p) => p.route === routeId);
}
