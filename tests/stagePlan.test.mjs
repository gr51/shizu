...
test('触发枚举真源：事件/动作集合与文档口径一致', () => {
  assert.equal(TRIGGER_EVENTS.size, 17);
  assert.equal(TRIGGER_ACTIONS.size, 17);
  for (const e of ['onStageClear', 'onLowHp', 'onTimeTick', 'onPlaneEnter', 'onDevour', 'onAmbushSpawn', 'onBossHalfHp', 'onCrisis']) assert.ok(TRIGGER_EVENTS.has(e));
